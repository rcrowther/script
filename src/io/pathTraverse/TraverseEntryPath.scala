    ...
    extends Traverse[Path]
{
  ...
  def newVisitor[U](f: (Path) => U) = new EntryVisitorPath(f)
}



object TraverseEntryPath
extends TraversePathCompanion[TraverseEntryPath]
 {

  protected val emptyThing = new TraverseEntryPath(
    emptyPath,
    false,
    0
  )
  ...
  def apply(
    ...
    depth: Int
      ...
      : TraverseEntryPath =
  {
    new TraverseEntryPath(path, followLinks, depth)
  }
