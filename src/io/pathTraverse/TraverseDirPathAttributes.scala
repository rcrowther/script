...
/** A collection of paths in a directory.
  ...
  */
class TraverseDirPathAttributes(
  val path: Path,
    ...
    extends Traverse[(Path, BasicFileAttributes)]
{
  ...
  def newVisitor[U](f: ((Path, BasicFileAttributes)) => U) = new DirVisitorPathAttributes(f)
}



object TraverseDirPathAttributes
extends TraversePathAttributesCompanion[TraverseDirPathAttributes]
 {

  protected val emptyThing = new TraverseDirPathAttributes(
    emptyPath,
    false,
    0
  )
      ...
      : Traversable[(Path, BasicFileAttributes)] =
  {
    ...
    new TraverseDirPathAttributes(path, followLinks, depth + 1).drop(1)
  }

}//TraverseDirPathAttributes
